...
public class FileDrop
{
        ...
        {   // Make a drop listener
            ...
            {   public void dragEnter( java.awt.dnd.DropTargetDragEvent evt)
                ...
                {   logger.info("FileDrop: drop event.");
                    ...
                    {   logger.info("FileDrop: IOException - abort:", io);
                        evt.rejectDrop();
                    }   // end catch IOException
                    catch (java.awt.datatransfer.UnsupportedFlavorException ufe) 
                    {   logger.info("FileDrop: UnsupportedFlavorException - abort:", ufe);
                        evt.rejectDrop();
        ...
        try { 
            ...
            while ((line = bReader.readLine()) != null) {
                try {
                    ...
                    logger.info("Error read line!", ex);
                }
            }

            return (java.io.File[]) list.toArray(new File[list.size()]);
        } catch (IOException ex) {
            logger.info("FileDrop exception!", ex);
        }
        return new File[0];
     }
     // END 2007-09-12 Nathan Blomquist -- Linux (KDE/Gnome) support added.
    ...
    private void makeDropTarget( final java.io.PrintStream out, final java.awt.Component c, boolean recursive)
    {
        ...
        {
            logger.info("FileDrop: Drop will not work due to previous error. Do you have another listener attached?");
            logger.error("Error while make drop target!", e);
        }   // end catch
    ...
    {   boolean ok = false;
        ...
        {   if( flavors.length == 0)
            ...
            for( i = 0; i < flavors.length; i++)
                logger.info(flavors[i].toString());
